...
async function getVersionFromBranch(branch: string): Promise<string> {
  if (isVercel) {
    const res = await fetch(
      `https://raw.githubusercontent.com/youyoumu/kiku/${branch}/packages/note/package.json`,
    );
...
export default async () => {
  const kikuVersionMain = await getVersionFromBranch("main");
  const kikuVersionDev = await getVersionFromBranch("dev");
